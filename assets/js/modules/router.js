/**
 * @file router.js
 * @description 全站路由配置模块。
 *
 * 职责：
 *   - 集中维护所有页面路由（路径、名称、标题）
 *   - 提供路由跳转、参数替换、当前路由检测等工具函数
 *   - 生成 HTML 链接字符串（routeLink）
 *
 * 设计说明：
 *   - BASE_PATH 留空支持站点根路径部署；如需子目录部署，修改此变量即可
 *   - 动态路由参数通过 ":paramName" 占位符定义，传入 params 对象时自动替换
 *   - 兼容 CommonJS 导出（供 Node.js 脚本或测试使用）
 */
